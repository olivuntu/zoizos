...
## ---- boxalti
	...
	par(mar = c(3, 6, 0.5, 0.5), mgp = c(3.5, 0.5, 0), tck = 0.03, las = 1)
	tmp <- ztax
...
## ---- boxwmalt
	par(mar = c(3, 5, 0.5, 0.5), mgp = c(3.5, 0.5, 0), cex = 1.2, cex.lab = 1.4, tck = 0.03, las = 1)
...
## ---- Stot
 par(mar = c(5, 5, 0.5, 0.5), mgp = c(2.5, 0.5, 0), tck = 0.03, las = 1, cex = 1.2, cex.lab = 1.4)
 tmp <- data.frame(S = Sn + Se, alti = as.numeric(names(Sn)))
 plot(S ~ alti, data = tmp, pch = 19, xlab = "Altitude (m)", ylab = "Total species richness", ylim = c(0, 20), cex = 1.1)
...
## ---- Snatexo
 ...
 x.polygon <- c(tmp$alti[i.for], tmp$alti[i.back])
 y.polygon <- c(ucl[i.for], lcl[i.back] )
	plot(S ~ alti, data = tmp, pch = 19, ylim = c(0, 20), xlab = "Elevation (m)", ylab = "Species richness", col = rgb(0.8, 0, 0.4), cex = 1.1)
 ...
 ucl <- fit + 1.96 * se
 y.polygon <- c(ucl[i.for], lcl[i.back] )
	points(S ~ alti, data = tmp, pch = 19, ylim = c(0, 15), xlab = "Elevation (m)", ylab = "Species richness", col = rgb(0, 0.4, 0.8), cex = 1.1)
...
## ---- Snatnul
	par(mar = c(5, 5, 0.5, 0.5), mgp = c(2.5, 0.5, 0), tck = 0.03, cex = 1.2, cex.lab = 1.4, las = 1)
	tmp <- data.frame(S = Sn, alti = as.numeric(names(Sn)))		
 lcl <- lowess(xnb[, 1], xnb[, "2.5%"], f = 1/4)
 ucl <- lowess(xnb[, 1], xnb[, "97.5%"], f = 1/4)
 x.polygon <- c(lcl$x, rev(ucl$x))
 y.polygon <- c(lcl$y, rev(ucl$y))
	plot(S ~ alti, data = tmp, ylim = c(0, 15), xlab = "Elevation (m)", ylab = "Species richness", pty = "n")
	polygon(x.polygon, y.polygon, col = rgb(0.7, 0.7, 0.7, alpha = 0.2), border = NA )
	polygon(x.polygon, y.polygon, col = rgb(0.3, 0.7, 0.3), density = 10, border = NA )
 ...
 y2.polygon <- c(lcl2$y, rev(ucl2$y))
	...
	polygon(x2.polygon, y2.polygon, col = rgb(0.3, 0.3, 0.9), density = 10, angle = -45, border = NA )
	points(S ~ alti, data = tmp, pch = 19, cex = 1.1)
 
 x <- tapply(Snrow, zhab$fal, mean)
 tmp$Smean <- x[!is.na(x)]
 points(Smean ~ alti, data = tmp, pch = 19, cex = 1.1, col = "red")
...
## ---- Sexonul
 ...
 ucl2 <- lowess(xec[, 1], xec[, "97.5%"], f = 1/4)
 x2.polygon <- c(lcl2$x, rev(ucl2$x))
 y2.polygon <- c(lcl2$y, rev(ucl2$y))
	polygon(x2.polygon, y2.polygon, col = rgb(0.7, 0.7, 0.7, alpha = 0.2), border = NA )
	polygon(x2.polygon, y2.polygon, col = rgb(0.3, 0.3, 0.9), density = 10, angle = -45, border = NA )
...
### ---- Snatgam")
# {
#		par(mar = c(5, 5, 0.5, 0.5), mgp = c(3.5, 0.5, 0), tck = 0.03, las = 1)
#		tmp <- data.frame(S = Sn, alti = as.numeric(names(Sn)))
#	 fit <- predict(modSn, type = "response")
#		se <- predict(modSn, se.fit = TRUE, type = "response")$se.fit
#	 lcl <- fit - 1.96 * se
